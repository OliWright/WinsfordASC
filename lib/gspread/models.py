...
class Worksheet(object):
    ...
    def new_cell(self, row, col):
        ...
        """
        url = construct_url('cells_cell_id', self, cell_id=self._cell_addr(row, col) )
        ...
        title_elem = SubElement(feed, 'title', {'type':'text'})
        title_elem.text = self.get_addr_int(row, col)
        
        SubElement(feed, 'link', {'rel': 'edit',
                                   'type': 'application/atom+xml',
                                   'href': url })
        
        SubElement(feed, 'gs:cell', { 'row':str(row), 'col':str(col), 'inputValue':'' } )
        
        # This is bonkers and there is definitely a much better way to do it,
        # but it looks like the cell constructor requires an ElementTree rather than simply
        # an element.
        feed = ElementTree.fromstring( "<?xml version='1.0' encoding='UTF-8'?>" + ElementTree.tostring(feed) )
    ...
    def update_cells(self, cell_list):
        ...
        """
        #logging.info( "Creating update feed" )
        feed = self._create_update_feed(cell_list)
        #logging.info( "Converting update feed to string" )
        feed_str = ElementTree.tostring(feed)
        #logging.info( "String is " + str(len(feed_str)) + " bytes" )
        #logging.info( "Posting update" )
        self.client.post_cells(self, feed_str)
        #logging.info( "Finished posting update" )
    ...
    def append_rows(self, rows):
        """Adds a rows to the worksheet and populates it with values.
        Widens the worksheet if there are more values than columns.
    ...
    def update_row(self, values, row_index=1):
        cell_list = []
        data_width = len(values)
        if self.col_count < data_width:
            self.resize(cols=data_width)
        ...
        for i, value in enumerate(values, start=1):
            cell = self.new_cell(row_index, i)
            cell.value = value
            cell_list.append(cell)
...
class Cell(object):
    ...
    def __init__(self, worksheet, element):
        ...
        #logging.info( "cell_elem: " + str(cell_elem) )
